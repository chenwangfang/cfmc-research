...
**公式D-3（最终公式）**：
```
概念复杂度 = 0.55 × D_abstract + 0.45 × D_processing
```
...
```
- F_field：领域转换度（同域=1，近域=2，远域=3）
- A_level：抽象层级（基本=1，上位=2，高度抽象=3）
- D_number：维度数量（单一=1，双重=2，多重=3）

**公式D-2：加工度维度**
```
D_processing = 0.6×I_depth + 0.4×S_schema
```
- I_depth：推理深度（直接=1，单步=2，多步=3）
- S_schema：图式激活（单一=1，组合=2，新创=3）

**示例zh55588验证**（"总路线是建设社会主义的锐利武器"，MD=2）：

| 参数 | 值 | 说明 |
|:-----|:--:|:-----|
| F_field | 2 | 政治→军事，近域转换 |
| A_level | 1 | 基本层级隐喻 |
| D_number | 1 | 单一维度 |
| I_depth | 1-2 | 直接/单步推理 |
| S_schema | 1 | 单一图式激活 |

计算：D_abstract = 0.25×2 + 0.50×1 + 0.25×1 = 1.25，D_processing ≈ 1.2，概念复杂度 ≈ 0.55×1.25 + 0.45×1.2 ≈ 1.23-2.0。标注值2.0与MD=2类型均值（2.002±0.853）一致。
