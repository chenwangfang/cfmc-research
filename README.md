...
本研究基于Sullivan (2013) 的隐喻构式理论，构建**汉语系表隐喻构式认知框架（CFMC: Cognitive Framework for Metaphorical Constructions）**，以5,971条发布标注记录为基础，采用**PLS-SEM形成性测量模型**检验以前三阶段为核心、语言编码为边界环节的四阶段认知编码机制，其中5,908条为核心隐喻构式样本。

### 研究目标

1. **类型特征**：建立基于认知通达度—映射方向双维度的12类构式分类体系
2. **网络组织**：揭示宏观类型网络中局部聚合与整体可达并存的描述性拓扑特征
3. **认知机制**：通过PLS-SEM形成性测量模型检验前三阶段认知编码路径及语言编码边界
...
- **网络模型**：两层构式网络（构式层+类型层）
- **认知模型**：以前三阶段为核心的四阶段认知编码机制PLS-SEM形成性测量模型检验

## 研究问题与假设

| 问题 | 内容 | 假设 | 章节 |
|:-----|:-----|:-----|:-----|
| **Q1** | 类型特征 | H1-1: 认知通达度与概念复杂度负相关 (*r* = -0.40至-0.60)<br>H1-2: CA×MD 12格操作分类获得限定支持（GMM/Bootstrap稳定性、CA×MD操作空间位置分层和概念复杂度相关效度证据达标，LDA作为压力测试提示类型边界） | 第5章 |
| **Q2** | 网络组织 | H2: 完整12类型无权宏观网络呈现描述性小世界特征 (*C*≥0.60, *L*≤3.0, σ>1)，扰动检验显示均值层面临界稳健 | 第6章 |
| **Q3** | 认知机制 | H3-1: PLS-SEM检验前三阶段路径及语言编码端点的边界关系，Y为系词功能名义类别的数值化边界指标 (GoF>0.25, 核心路径系数显著 *p*<.05; PLS-MGA按系词功能分3组检验前三阶段路径的跨功能共享性)<br>H3-2: 原型距离、认知通达度与四阶段指标之间的分层桥梁关联；9类构式Pearson近似相关仅作为探索性补充 | 第7章 |
...
本研究采用**PLS-SEM形成性测量模型**检验四阶段认知编码机制（η₁域激活 → η₂参照点锚定 → η₃跨域映射 → Y语言编码）。其中前三阶段是正式支持对象，Y为系词功能名义类别的数值化边界指标。
