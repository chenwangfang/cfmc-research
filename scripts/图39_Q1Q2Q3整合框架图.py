...
def load_data():
    ...
    # 读取表93_SEM模型拟合指标比较（H3-1 CFI）
    with open(os.path.join(data_dir, '表93_SEM模型拟合指标比较.json'), 'r', encoding='utf-8') as f:
        table93 = json.load(f)
    
    # 读取表104_认知通达度与阶段1-2相关分析（H3-2）
    with open(os.path.join(data_dir, '表104_认知通达度与阶段1-2相关分析.json'), 'r', encoding='utf-8') as f:
        table104 = json.load(f)
    ...
    # H3-1 CFI：从表93提取优化模型的CFI
    for item in table93:
        if item.get('指标') == 'CFI':
            data['cfi'] = float(item['优化模型（eta2->eta3）'])
            break
    
    # H3-2相关系数范围：从表104提取前两个r值
    r_values = []
    for item in table104[:2]:
        r = item.get('相关系数r')
        if r and r != '-':
            r_values.append(abs(float(r)))
    if r_values:
        data['h3_2_r_min'] = min(r_values)
        data['h3_2_r_max'] = max(r_values)
...
def create_figure_8_1(data):
    ...
    y -= LINE_SPACE['subtitle_to_content']
    # 动态数据：小世界性质
    ax.text(1.8, y, f'• 小世界性质：C={data["C"]:.2f}, L={data["L"]:.2f}, σ={data["sigma"]:.2f}',
            fontsize=FONTS['content'], ha='left')
    ...
    ax.text(1.8, y, '• 中心性分析：枢纽节点连接各模块',
            fontsize=FONTS['content'], ha='left')
    y -= LINE_SPACE['content_to_hypo']
    ax.text(5, y, 'H2: C≥0.60 [√], L≤3.0 [√], σ>1 [√]',
    ...
    ax.text(1.8, y, '• 四阶段编码机制   • 测量不变性验证   • 路径强度分析',
            fontsize=FONTS['content'], ha='left')
    y -= LINE_SPACE['content_to_hypo']
    # 动态数据：H3-1 CFI 和 H3-2 相关系数
    ax.text(5, y, f'H3-1: CFI={data["cfi"]:.3f} [√]    H3-2: |r|={data["h3_2_r_min"]:.2f}-{data["h3_2_r_max"]:.2f} [√]',
...
def get_output_dir():
    ...
    if system == 'Windows':
        ...
        if os.path.exists('/mnt/c/Windows'):
            return '/mnt/e/博士毕业论文/大论文/论文撰写/统计分析/结果_输出/Figures'
        else:
...
def main():
    ...
    print(f"   Q2: C={data['C']:.2f}, L={data['L']:.2f}, σ={data['sigma']:.2f}")
    print(f"   H3-1: CFI={data['cfi']:.3f}")
    print(f"   H3-2: |r|={data['h3_2_r_min']:.2f}-{data['h3_2_r_max']:.2f}")
    ...
    png_path = os.path.join(output_dir, "图39_Q1Q2Q3研究发现整合框架.png")
