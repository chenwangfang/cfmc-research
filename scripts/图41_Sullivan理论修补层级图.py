...
def load_data():
    ...
    # C2证据：从表94提取关键路径系数
    for item in table94:
        if 'eta2->eta3' in item.get('路径', ''):
            data['beta2'] = float(item['系数beta'])
        elif 'eta2->cognitive_accessibility' in item.get('路径', ''):
            data['beta_ca'] = float(item['系数beta'])
...
def main():
    ...
    print(f"   C2证据: CFI={data['cfi']:.3f}, β(eta2->eta3)={data['beta2']:.3f}")
    ...
    ax.text(6, 7.25, '细化路径：两步程序 → 四阶段认知编码机制', fontsize=12,
            ha='center', va='center', color='#333', fontproperties=font_prop)
    # 动态数据：C2 SEM指标
    ax.text(6, 6.75, f'量化验证：SEM建模（CFI = {data["cfi"]:.3f}，β = {data["beta2"]:.3f}）', fontsize=12,
            ha='center', va='center', color='#333', fontproperties=font_prop)
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/图41_Sullivan理论修补层级图.png", dpi=1200, bbox_inches='tight',
                facecolor='white', edgecolor='none')

    print("✅ 图41已保存至:")
    print(f"  - {output_dir}/图41_Sullivan理论修补层级图.png")
