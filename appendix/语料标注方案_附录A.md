...
**说明**：系词类型 (copula_type) 标注系词的形态类别（基本系词如"是"、扩展系词如"成为"、否定系词如"不是"、零系词）。注意区分：copula_type是本表的标注字段，测量系词的形态类型；copula_function是SEM模型的结果变量X₁₁，编码为1-3数值变量 (1=equative等同, 2=attributive属性, 3=identificational识别)，反映跨域映射 (*η*₃) 的语言编码结果。
