...
- **Q1 (类型体系)**：汉语系表隐喻构式呈现哪些类型及其特征？该问题采用双维度分类体系 (认知通达度×映射方向)，识别12类构式并验证其原型梯度结构。
...
本研究采用CFMC-33标注体系，将隐喻构式认知框架(CFMC)转化为可操作的标注规范。CFMC-33共包含41项字段 (33项必填+8项选填)，采用四层级结构。完整的字段定义、取值范围和编码规范详见附录B。
...
**设计原则**：CFMC-33的字段设计遵循"理论驱动+研究问题导向"原则。Level 1核心字段 (23项)直接服务于Q1-Q3三个研究问题的假设检验；Level 2补充字段 (4项)测量汉语语法特点，其中holistic_imagery(整体性意象)和relational_thinking(关系性思维)用于检验汉语认知特色的调节效应；Level 3-4选填字段 (8项)用于典型案例的深度分析。

#### 1.4 四阶段认知编码机制

CFMC-33的核心设计依据是四阶段认知编码机制，该机制将Sullivan (2013) 的两步描述发展为可检验的形式化模型。四阶段机制采用11个CFMC-33字段作为SEM测量指标。
...
- **概念复杂度**：通过映射方向操作化为4类 (具体→具体、具体→抽象、抽象→抽象、抽象→具体)

双维度交叉形成3×4=12类构式，呈现原型梯度结构(中心—次中心—边缘)。

### 第二部分：字段体系说明

#### 2.1 CFMC-33字段体系概览

CFMC-33 (Cognitive Framework for Metaphorical Constructions-33) 是本研究构建的汉语系表隐喻构式标注字段体系，共包含41项字段 (33项必填+8项选填)。该体系将本研究构建的CFMC理论框架转化为可操作的标注规范。
...
基础字段至Level 2共33项必填字段构成标注核心，Level 3-4的8项选填字段用于深度个案分析。其中，Level 1核心字段 (23项)直接服务于Q1-Q3三个研究问题的假设检验，Level 2补充字段 (4项)测量汉语语法特点。
...
3. **汉语类型学特征**：实现语言适应，零系词构式、话题突出结构、整体性思维等汉语特色对应G区字段 (holistic_imagery, relational_thinking)和F区字段 (copula_type设置"零系词"选项)。
...
四个阶段形成级联加工关系：阶段1→阶段2→阶段3→阶段4，共采用11个CFMC-33字段作为SEM测量指标。

#### 2.4 双维度分类体系与字段对应

双维度分类体系是本研究为回答Q1 (类型体系)而构建的核心分析工具，基于Sullivan自主—依存原则发展出两个对称维度。
...
- 抽象→抽象 (抽抽)：高阶抽象映射
- 抽象→具体 (抽具)：逆向映射
- 核心字段：mapping_direction, source_domain, target_domain
...
| | 常规度指标 | conventionality | 定距 (1-5) |
| | 新颖度指标 | metaphor_novelty | 定距 (1-5) |
| 概念复杂度 | 映射方向分类 | mapping_direction | 定类 (4类) |
...
|:---------|:---------|:----------------|
| Q1类型体系 | 认知通达度 | cognitive_accessibility, conventionality, metaphor_novelty |
...
部分字段同时服务于多个研究问题。例如，source_domain和target_domain既用于Q1的概念复杂度分类，也用于Q3阶段1的认知域激活测量；cognitive_accessibility既是Q1双维度分类的核心变量，也是Q3阶段2的测量指标；prototype_distance既反映Q1的原型梯度结构，也作为Q3的认知参照点锚定指标。这种字段复用体现了CFMC-33的设计经济性：41项字段充分覆盖三个研究问题的分析需求。
...
Level 1字段为核心必填项，覆盖五大类别 (A-E)，支撑四阶段认知编码机制的分析。这些字段直接对应SEM模型的11个观测指标：
...
- **阶段三 (跨域映射)**：mapping_direction(映射方向)、mapping_basis(映射基础)、systematicity(系统性)、entailment_richness(蕴涵丰富度)
...
| 字段名 | 英文名 | 数据类型 | 取值范围/说明 |
|:-------|:-------|:---------|:--------------|
| 构式类型 | construction_type | String | copular_metaphor/copular_simile/other |
| 主语 | subject | String | 主语成分 (NP₁，唤起目标域) |
| 系词 | copula | String | 系词列表 (是/为/即/成/等于等) |
| 表语 | predicate | String | 表语成分 (NP₂，唤起源域) |

**说明**：A类字段识别系表隐喻构式的基本句法成分。主语 (NP₁)与目标域对应，表语 (NP₂)与源域对应，体现"NP₁是NP₂"的核心构式形态。
...
| 映射类型 | mapping_type | String | 结构映射/属性映射/关系映射 |
| 隐喻层级 | metaphor_level | String | 初级隐喻/复合隐喻/概念隐喻 |
| 具体度_源域 | concreteness_source | Integer | 1-5级 (1=极抽象，5=极具体) |
| 具体度_目标域 | concreteness_target | Integer | 1-5级 (1=极抽象，5=极具体) |

**说明**：B类字段描述隐喻的概念结构。源域和目标域是概念隐喻的核心要素；mapping_direction是双维度分类的关键维度，与认知通达度共同决定构式的12类归属。
...
| 概念复杂度 | conceptual_complexity | Integer | 1-5级 (1=极低，5=极高) |
| 常规度 | conventionality | Integer | 1-5级 (1=创新，5=高度规约) |

**说明**：C类字段测量认知加工的核心维度。认知通达度和概念复杂度构成双维度分类的基础 (3级CA × 4类映射方向 = 12类构式)。常规度反映构式的规约化程度。详细测量方案见附录C和附录D。
...
| 蕴涵丰富度 | entailment_richness | Integer | 1-5级 (1=极低，5=极高) |
| 映射基础 | mapping_basis | String | 相似性/相关性/文化约定/经验关联 |

**说明**：D类字段描述认知生成路径的特征。embodied_experience测量隐喻与具身经验的关联程度；entailment_richness测量映射所携带的推理潜力，是阶段3跨域映射的核心指标。
...
| 字段名 | 英文名 | 数据类型 | 取值范围/说明 |
|:-------|:-------|:---------|:--------------|
| 链接类型 | link_type | String | 隐喻扩展链接/多义链接/实例链接/子部分链接 |
| 原型距离 | prototype_distance | Integer | 1-3级 (1=中心，2=次中心，3=边缘) |
| 系统性 | systematicity | Integer | 1-5级 (1=极低，5=极高) |
| 构式间链接 | inter_construction_links | String | 关联构式标识列表 |
| 网络功能 | function_in_network | Integer | 1-5级 (1=边缘，5=核心枢纽) |
| 常规度 | conventionality | Integer | 1-5级 (详见C区表A-10，此处因其影响网络位置而列入) |

**说明**：E类字段描述构式的网络属性，支撑Q2构式网络组织研究。link_type对应Goldberg四类链接关系；prototype_distance用于验证原型梯度；inter_construction_links和function_in_network支撑网络拓扑分析；conventionality (常规度)影响构式在网络中的位置 (高规约化构式趋向网络中心)，该字段在C区已有详细定义。

#### 3.3 Level 2补充字段 (4项)

Level 2字段为非必要但重要的补充信息，用于捕捉汉语系表隐喻构式的语言特色和认知细节。
...
| 字段名 | 英文名 | 数据类型 | 取值范围/说明 |
|:-------|:-------|:---------|:--------------|
| 系词类型 | copula_type | String | 基本/扩展/否定/零 |

**说明**：系词类型 (copula_type) 标注系词的形态类别（基本系词如"是"、扩展系词如"成为"、否定系词如"不是"、零系词）。注意区分：copula_type是本表的标注字段，测量系词的形态类型；copula_function是SEM模型的结果变量X₁₁，编码为1-3定序量表 (1=equative等同, 2=predicational述谓, 3=specificational指称)，反映跨域映射 (*η*₃) 的语言编码结果。
...
| 字段名 | 英文名 | 数据类型 | 取值范围/说明 |
|:-------|:-------|:---------|:--------------|
| 隐喻新颖度 | metaphor_novelty | Integer | 1-5级 (1=高度规约，5=高度创新) |

**说明**：隐喻新颖度作为认知通达度 (CA) 的补充测量指标，采用1-5定距量表，测量隐喻表达的规约化程度与创新程度。
...
2. **Level 1必填** (23项)：覆盖五大核心类别，支撑四阶段认知编码机制的11个SEM指标提取
...
**A_基础识别 (4项)**：识别构式的句法成分
- construction_type：判断构式类型(copular_metaphor/copular_simile/other)
- subject：提取主语成分(NP₁)
- copula：识别系词
- predicate：提取表语成分(NP₂)
...
- mapping_type：判断映射类型 (结构/属性/关系映射)
- metaphor_level：确定隐喻层级 (初级/复合/概念隐喻)
- concreteness_source/concreteness_target：评估源域和目标域的具体度 (1-5级)

**C_认知机制 (3项)**：评估认知加工维度
- cognitive_accessibility：按1-5级评定认知通达度
- conceptual_complexity：按1-5级评定概念复杂度
- conventionality：按1-5级评定常规度

**D_认知生成路径 (3项)**：描述认知生成路径特征
- embodied_experience：评估具身经验相关度 (0.0-1.0连续评分)
- entailment_richness：评估蕴涵丰富度 (1-5级)
- mapping_basis：判断映射基础 (相似性/相关性/文化约定/经验关联)

**E_网络关系 (6项)**：标注构式网络属性
- link_type：确定链接类型 (隐喻扩展/多义/实例/子部分链接)
- prototype_distance：评定原型距离 (1=中心，2=次中心，3=边缘)
- systematicity：评估系统性程度 (1-5级)
- inter_construction_links：记录关联构式标识列表
- function_in_network：评定网络功能 (1=边缘，5=核心枢纽)
- conventionality：评定常规度 (1-5级，详见C区，影响网络位置)

##### 4.2.3 阶段3：Level 2标注

**F_句法特征 (1项)**
- copula_type：系词类型 (判断/存在/变化/比拟)
...
**H_认知细化 (1项)**
- metaphor_novelty：隐喻新颖度 (1-5级，1=高度规约，5=高度创新)
...
1. **源域-目标域一致性**：映射方向为"具体→抽象"时，源域具体度应高于目标域
2. **通达度-复杂度关联**：高认知通达度通常对应低概念复杂度
3. **原型距离验证**：中心成员(prototype_distance=1)应具有高通达度和低复杂度
...
1. **认知域激活**：识别源域和目标域(B类字段)
2. **认知参照点锚定**：评估通达度和复杂度 (C类字段)
3. **跨域映射**：分析映射方向和基础(D类字段)
4. **网络关联**：标注构式网络属性(E类字段)
...
**说明**：两个维度独立判定，共同构成双维度分类的基础。详细测量方案见附录C和附录D。
